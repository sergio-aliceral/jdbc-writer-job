package br.com.aliceraltecnologia.jdbcwriterjob.writer;

import org.springframework.batch.item.file.FlatFileItemWriter;
import org.springframework.batch.item.file.builder.FlatFileItemWriterBuilder;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;
import org.springframework.core.io.FileSystemResource;

import br.com.aliceraltecnologia.jdbcwriterjob.dominio.Conta;

@Configuration
public class ClienteValidoFileWriterConfig {

	@Bean
	public FlatFileItemWriter<Conta> clienteValidoFileWriter() {
		return new FlatFileItemWriterBuilder<Conta>()
				.name("clienteValidoFileWriter")
				.resource(new FileSystemResource("./arquivos/conta.txt"))
				.delimited()
				.names("tipo", "limite", "clienteId")
				.build();
	}
}
